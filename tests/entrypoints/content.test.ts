...
describe('content runtime messages', () => {
  ...
  it('keeps the explicit status when an older automatic request finishes later', async () => {
    ...
    let release!: (value: unknown) => void; let calls = 0; const sendMessage = vi.fn(() => { calls += 1; return calls === 1 ? new Promise((resolve) => { release = resolve; }) : Promise.resolve([{ mode: 'digits', text: '2', confidence: .9 }]); });
    ...
    content.enable(); const automatic = content.workflow.run(image, 'automatic'); await new Promise((resolve) => setTimeout(resolve, 0)); expect(sendMessage).toHaveBeenCalledOnce();
