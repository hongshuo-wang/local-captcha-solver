...
算式可以使用 `=?`、`=`、`?` 或无后缀形式。减法结果必须非负，除法必须整除。图片选择、动画、行为验证、非拉丁文字、小数、余数、负数结果和多步数学不在静态 OCR 范围内。

独立的拼图滑块 Beta 支持桌面 Chrome/Edge、单个缺口、水平轨道和可稳定读取视觉资源的挑战。它采用通用定位逻辑，并使用 [GeeTest 演示页](https://2captcha.com/demo/geetest) 进行验证。在设置中可以选择所有 HTTP/HTTPS 网站，或只选择精确网站名单。处理滑块会使用浏览器的 `debugger` 权限发送可信拖动，浏览器可能在安装或升级时显示确认提示。遇到结构不明确、挑战变化、定位置信度不足或服务端拒绝时会停止并交还用户。
...
3. 如果浏览器提示，请确认 `debugger` 权限。它只在手动处理或已开启自动处理的网站上发送拖动，单次操作结束后立即断开。
4. 使用 [GeeTest 演示页](https://2captcha.com/demo/geetest) 测试。

插件只会在启用的网站等待唯一可见滑块，确认页面和用户操作稳定后再拖动；定位或结果不确定时会停止，不会猜测。

### 升级到 1.2.0

从旧版本升级时，插件会单独打开一次滑块功能说明页。你可以在页面中配置滑块运行范围、查看权限摘要，并在独立 Tab 打开 GeeTest 演示页体验。原有静态 OCR 设置会继续保留。
