...
type ViewName = 'access' | 'behavior' | 'diagnostics' | 'about';
...
function shell(t: Translator): string {
  return `<div class="settings-shell">
    <aside class="settings-sidebar">
      <a class="brand" href="#access" data-nav="access" aria-label="Captcha Helper">
    ...
    <main class="settings-content" id="content">
      <section class="view" data-view="access"></section>
      <section class="view" data-view="behavior" hidden></section>
...
function accessMarkup(settings: CaptchaSettings, globalGranted: boolean, selectedAccess: readonly SelectedSiteAccess[], t: Translator): string {
  ...
  return `${pageHeading(t('navSettings'), t('accessHeading'), t('accessDescription'))}
    <div class="content-section access-section">
      ...
      <p class="inline-status" data-settings-status role="status"></p>
...
function behaviorMarkup(settings: CaptchaSettings, t: Translator): string {
  return `${pageHeading(t('navSettings'), t('recognitionHeading'), t('behaviorDescription'))}
    ...
    <section class="content-section language-section">
      ...
      <label class="setting-row select-row" for="interface-locale"><span><strong>${t('languageLabel')}</strong><small>${t('languageSystem')}</small></span><select id="interface-locale"><option value="system" ${settings.interfaceLocale === 'system' ? 'selected' : ''}>${t('languageSystem')}</option><option value="zh_CN" ${settings.interfaceLocale === 'zh_CN' ? 'selected' : ''}>${t('languageChinese')}</option><option value="en" ${settings.interfaceLocale === 'en' ? 'selected' : ''}>${t('languageEnglish')}</option></select></label>
    </section>`;
}
...
export async function startOptions(
  ...
  const currentView = (): ViewName => {
    ...
    return hash === 'behavior' || hash === 'diagnostics' || hash === 'about' ? hash : 'access';
  };
  ...
  const show = async (view: ViewName): Promise<void> => {
    ...
    required<HTMLElement>(root, '[data-view="access"]').innerHTML = accessMarkup(settings, globalGranted, selectedAccess, t);
    required<HTMLElement>(root, '[data-view="behavior"]').innerHTML = behaviorMarkup(settings, t);
  ...
  const selectMode = async (mode: 'all' | 'selected'): Promise<void> => {
    if (mode === 'all') {
      ...
      if (!granted) { setInlineStatus('[data-settings-status]', t('permissionDenied'), true); return; }
  ...
  const wire = (view: ViewName): void => {
    ...
    root.querySelector<HTMLButtonElement>('[data-open-guide]')?.addEventListener('click', () => {
      void settingsStore.setOnboardingComplete(false).then(() => navigate(extension.runtime.getURL('onboarding.html')));
    });
    ...
    root.querySelector<HTMLButtonElement>('[data-revoke-global]')?.addEventListener('click', () => { void extension.permissions.remove({ origins: [...GLOBAL_HTTP_ORIGINS] }).then(reconcile).then(() => show('access')); });
    root.querySelector<HTMLFormElement>('[data-site-form]')?.addEventListener('submit', (event) => {
      ...
      void (async () => {
        let hostname: string;
        try { hostname = parseHostname(input.value); } catch { setInlineStatus('[data-settings-status]', t('invalidHostname'), true); return; }
        ...
        if (!granted) { setInlineStatus('[data-settings-status]', t('permissionRequestFailed'), true); return; }
    ...
    root.querySelectorAll<HTMLButtonElement>('[data-remove-site]').forEach((button) => button.addEventListener('click', () => {
      ...
      void settingsStore.removeSelectedSite(rule).then(() => extension.permissions.remove({ origins: [...originsForSelectedSite(rule)] })).then(reconcile).then(() => show('access'));
    }));
