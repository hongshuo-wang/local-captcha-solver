...
export function createCaptchaWorkflow(options: CaptchaWorkflowOptions): CaptchaWorkflow {
  ...
  async function execute(image: HTMLImageElement, first: ImageDetailSnapshot, record: RequestRecord, trigger: WorkflowTrigger): Promise<WorkflowResult> {
    ...
    if (!chosen.safe || options.autoFillEnabled?.() === false) return { state: 'needs_confirmation', candidateId, displayText: selected.displayText, fillValue: selected.fillValue, fieldIds: match.state === 'unique' ? [match.winner.id] : match.candidates.map((item) => item.field.id) };
    ...
    if (target.element.value !== '') return { state: 'needs_confirmation', candidateId, displayText: selected.displayText, fillValue: selected.fillValue, fieldIds: [target.id] };
    const fill = fillEmptyField(target.element, selected.fillValue);
    return fill.state === 'filled' ? { state: 'filled', candidateId, fieldId: target.id, displayText: selected.displayText, fillValue: selected.fillValue } : fill.state === 'stale' ? stale(candidateId) : fill.state === 'not_empty' ? { state: 'needs_confirmation', candidateId, displayText: selected.displayText, fillValue: selected.fillValue, fieldIds: [target.id] } : { state: 'no_field', candidateId, displayText: selected.displayText, fillValue: selected.fillValue };
  }
