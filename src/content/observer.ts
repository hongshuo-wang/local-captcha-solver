...
export function observeCaptchaImages(workflow: CaptchaWorkflow, root: Document = document): CaptchaObserver {
  ...
  const affectsContext = (node: Element): boolean => relevant(node) || node.closest('input, label, form, fieldset, select, textarea') !== null || node.querySelector('img, input, label, form, fieldset, select, textarea') !== null;
  ...
  const observer = new MutationObserver((records) => records.forEach((record) => { const target = contextNode(record.target); if (record.type === 'attributes') { if (target && affectsContext(target)) refreshContext(target); return; } if (record.type === 'characterData') { if (target && affectsContext(target)) refreshContext(target); return; } const additions = [...record.addedNodes].map(contextNode).filter((node): node is Element => node !== undefined && !node.closest('[data-local-captcha-status]')); additions.forEach(scan); const changedRelevant = additions.some(affectsContext) || [...record.removedNodes].some((node) => node instanceof Element && affectsContext(node)); if (target && (changedRelevant || relevant(target))) refreshContext(target); }));
