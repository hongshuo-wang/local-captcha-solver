...
export function observeCaptchaImages(workflow: CaptchaWorkflow, root: Document = document): CaptchaObserver {
  ...
  const contextNode = (node: Node): Element | undefined => node instanceof Element ? node : undefined;
  const observer = new MutationObserver((records) => records.forEach((record) => { if (record.type === 'attributes') { if (record.target instanceof Element) refreshImages(record.target); return; } const additions = [...record.addedNodes].map(contextNode).filter((node): node is Element => node !== undefined && !node.closest('[data-local-captcha-status]')); additions.forEach(scan); const relevant = additions.some((node) => node instanceof HTMLImageElement || node.matches('input, label, form, select, textarea')); if (relevant && record.target instanceof Element) refreshImages(record.target); }));
  observer.observe(root.documentElement, { subtree: true, childList: true, attributes: true, attributeFilter: ['src', 'srcset', 'class', 'style', 'hidden'] });
