...
export function observeCaptchaImages(workflow: CaptchaWorkflow, root: Document = document): CaptchaObserver {
  ...
  const refreshImages = (scope: ParentNode) => { const images = scope instanceof HTMLImageElement ? [scope] : [...scope.querySelectorAll('img')]; images.forEach((image) => { processed.delete(image); workflow.invalidate?.(image); schedule(image); }); };
  ...
  const refreshContext = (node: Element) => { if (node.closest('[data-local-captcha-status]')) return; const scope = node instanceof HTMLImageElement || node.querySelector('img') !== null ? node : node.closest('form, fieldset') ?? node.parentElement; if (scope) refreshImages(scope); };
  const observer = new MutationObserver((records) => records.forEach((record) => { const target = contextNode(record.target); if (record.type === 'attributes') { if (target) refreshContext(target); return; } if (record.type === 'characterData') { if (target) refreshContext(target); return; } const changed = [...record.addedNodes, ...record.removedNodes].map(contextNode).filter((node): node is Element => node !== undefined && !node.closest('[data-local-captcha-status]')); changed.forEach(scan); if (changed.some(relevant) && target) refreshContext(target); }));
  observer.observe(root.documentElement, { subtree: true, childList: true, characterData: true, attributes: true, attributeFilter: ['src', 'srcset', 'class', 'style', 'hidden', 'disabled', 'readonly', 'aria-label', 'placeholder', 'name', 'id'] });
