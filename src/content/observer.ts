...
export function observeCaptchaImages(workflow: CaptchaWorkflow, root: Document = document): CaptchaObserver {
  ...
  const refreshImages = (scope: ParentNode) => { const images = scope instanceof HTMLImageElement ? [scope] : [...scope.querySelectorAll('img')]; images.forEach((image) => { processed.delete(image); schedule(image); }); };
  const observer = new MutationObserver((records) => records.forEach((record) => { if (record.type === 'attributes') { if (record.target instanceof Element) refreshImages(record.target); return; } record.addedNodes.forEach((node) => { if (node instanceof Element && !node.closest('[data-local-captcha-status]')) scan(node); }); if (record.addedNodes.length > 0 && ![...record.addedNodes].some((node) => node instanceof Element && node.closest('[data-local-captcha-status]'))) refreshImages(root); }));
