...
function show(message: string, anchor?: Element, action?: { label: string; onClick: () => void }): void {
  ...
  document.body.append(region); removalTimer = setTimeout(() => region.remove(), action ? 8000 : 4000);
}
export function showRecognizing(anchor?: Element): void { show('正在识别验证码…', anchor); }
